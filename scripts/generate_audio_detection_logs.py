...
def generate_session_log(conn: sqlite3.Connection, out_path: Path) -> int:
    query = """
    SELECT 
        ...
        (SELECT COUNT(*) FROM audio_flags WHERE s_id = s.s_id AND status = 'ACTIVE') as n_flags,
        (SELECT MAX(conf) FROM audio_flags WHERE s_id = s.s_id AND status = 'ACTIVE') as max_conf
    ...
    # Pre-fetch active flags for categories mapping
    flags_query = """
    SELECT s_id, intent FROM audio_flags WHERE status = 'ACTIVE'
    """
    flags_data = conn.execute(flags_query).fetchall()
    session_flags = {}
    for r in flags_data:
        session_flags.setdefault(r[0], Counter())[r[1]] += 1
    ...
    for r in rows:
        ...
        counter = session_flags.get(sid, Counter())
        ...
        out_rows.append({
            ...
            "n_segments": r[5],
            "n_flags": r[6],
            "flag_categories": _format_flag_categories(counter),
            "max_confidence": f"{r[7]:.2f}" if r[7] is not None else "",
        })
...
def generate_detailed_log(conn: sqlite3.Connection, out_path: Path) -> int:
    query = """
    SELECT 
...
def main() -> None:
    ...
    if args.mode in ("session", "both"):
        out_path = out_dir / f"audio_detection_logs_{stamp}.csv"
        print(f"Generating session-level log -> {out_path.name}...")
        n = generate_session_log(conn, out_path)
        print(f"  Written {n} rows")
        
    if args.mode in ("detailed", "both"):
        out_path = out_dir / f"audio_detection_logs_detailed_{stamp}.csv"
        print(f"Generating detailed log -> {out_path.name}...")
        n = generate_detailed_log(conn, out_path)
